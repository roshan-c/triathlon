  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-lg items-center px-6 py-12">
      <section className="w-full rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow-md)] sm:p-8">
        ...
        <h1 className="mt-4 text-2xl font-semibold tracking-tight text-[var(--foreground)]">Create your next project</h1>
        <p className="mt-2 text-sm text-[var(--muted-foreground)]">
          Projects include a default board and columns so your team can start immediately.
        </p>

        {projects.length > 0 ? (
          <p className="mt-3 text-xs text-[var(--muted-foreground)]">
            You currently have access to {projects.length} project(s).
          </p>
        ) : null}
        ...
        {canCreateProjects ? (
          <form onSubmit={submit} className="mt-6 space-y-4">
            <label className="block text-sm">
              <span className="mb-1.5 block text-xs font-medium text-[var(--muted-foreground)]">Project name</span>
              <input
                value={name}
            ...
            <label className="block text-sm">
              <span className="mb-1.5 block text-xs font-medium text-[var(--muted-foreground)]">Description (optional)</span>
              <textarea
                value={description}
          ...
          <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--background-alt)] p-4">
